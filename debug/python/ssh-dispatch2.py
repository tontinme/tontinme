...
def launcher(i,q,cmd):
	"""
	Spawns command in a thread to an ips
	"""
	while True:
...
#Determin Number of threads to use, but max out at 25
if len(ips) < 25:
	num_threads = len(ips)
else:
	num_threads = 25
...
#print "Dispath completed in %f seconds" % end - start
print "Dispath completed in %s - %s seconds" % (end, start)
